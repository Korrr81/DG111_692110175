...
int loadGame(const char *path, GameData *d)
{
    ...
    GameData tmp;
    size_t n = fread(&tmp, sizeof(GameData), 1, f);
    fclose(f);
    if (n != 1)
        return -1;
...
void randomAction(GameData *d)
{
    ...
    if (d->hp <= 0)
    {
        printf("You died! Respawn at (100,100) with full HP, scorehalved\n");
        d->x = 100.0f;
        d->y = 100.0f;
        d->hp = d->max_hp;
        d->score /= 2;
...
int main()
{
    ...
    while (1)
    {
        printf("1.Save, 2.Load, 3.Random_action, 4.Show_state, 5.Quit: ");
        if (scanf("%d", &choice) != 1)
        {
            int c;
            while ((c = getchar()) != '\n' && c != EOF)
